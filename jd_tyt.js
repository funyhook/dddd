...
#by:微信公众号：玩机匠 自定义互助码变量，魔改支持多个互助码
export TYT_PACKETID="互助码1&互助码2&互助码3"
 [task_local]
#快速推一推
...
if ($.isNode() && process.env.TYT_PACKETID) {
  tytpacketId = process.env.TYT_PACKETID.split('&');
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if ($.canRun) {
        ...
        await $.wait(500)
        if (tytpacketId.length>0) {
            for (let i = 0; i < tytpacketId.length; i++) {
                await $.wait(1000);
                await tythelp(tytpacketId[i]);
            }
        }
      }
    }
